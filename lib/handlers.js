...
handlers.favicon = function(data,callback){
  // Reject any method that is't a GET
  if(data.method == 'get'){
...
handlers._users.get = function(data, callback) {
  ...
  const phone =
    ...
    data.queryStringObject.phone.trim().length > 8
                    ...
                    _data.delete("checks", checkId, function(err) {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if(!deletionErrors){
                          ...
                          callback(500, {'Error': 'Errors ecountered while attempting to delete all of the user\'s checks. All checks may not have been deleted from the system successfully'});
                        }
  ...
  if (phone && password) {
    ...
    _data.read("users", phone, function(err, userData) {
      if (!err && userData) {
        ...
        callback(400, { Error: "Could not find the specitied user" });
      }
    });
  } else {
...
// Checks - post
// Requested data: protocol, url, method, successCodes, timeoutSeconds
// Optional data: none
...
handlers._checks.put = function(data, callback) {
  ...
  if (id) {
    // Check to make sure one or more optional fielda has been set
    if (protocol || url || method || successCodes || timeoutSeconds) {
      // Lookup the check
      _data.read("checks", id, function(err, checkData) {
        if (!err && checkData) {
              ...
              if (!err) {
                ...
                _data.read("users", checkData.userPhone, function(
                  ...
                  if (!err && userData) {
                    ...
                    callback(500, {
                      Error:
                        "Could not find the user who created the check, so could not delete the check"
                    });
                  }
                });
              } else {
