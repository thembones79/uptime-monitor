...
workers.validateCheckData = function(originalCheckData) {
  ...
  if (
    ...
    console.log(
      "Error: One of the checks in not properly formatted. Skipping it."
    );
  }
};

// Perform the check, send the originalCheckData and the outcome of the check to another step of the process
workers.performCheck = function(originalCheckData) {
  ...
  let path = parsedUrl.path; // Using path and not "pathname" beacuse we want the query string
...
workers.rotateLogs = function() {
  ...
  _logs.list(false, function(err, logs) {
    if (!err && logs && logs.legth > 0) {
      logs.forEach(function(logName) {
        // Compress the data to a diffrent file
        let logId = logName.replace(".log", "");
        let newFieldId = logId + "-" + Data.now();
        _logs.compress(logId, newFileId, function(err) {
          if (!err) {
            // Truncate the log
            _logs.truncste(logId, function(err) {
              if (!err) {
